...
**완료 (2026-09-26).** 계획대로 `startEdit()` 하나. 계획과 다르게 한 것: 옛 백업에서 온 기록은 이름·메모가 비어(undefined) 있을 수 있어 `''`로 채운다 — 입력칸이 비제어로 바뀌지 않게 (빈칸 채우기의 근본은 작업 8). 화면을 처음 열 때 기록이 아직 읽히는 중이면 입력값이 빈 값으로 굳던 길도 같이 사라졌다. 브라우저 확인: 수정 → 이름·메모를 고치고 삭제 확인을 연 채 취소 → 다시 수정 → 원래 이름·빈 메모, 삭제 확인 닫힘. **AI 경로(이 이름으로 → 수정 → 메모만 저장)는 개발 서버에 판정 AI가 없어 브라우저에서 확인하지 못했다** — 실제 사이트에서 한 번 본다.
