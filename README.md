...
| 기록 저장(IndexedDB) · 목록 · 상세 · 수정 · 삭제 | 동작. 새로고침해도 남는다. '수정'은 열 때마다 지금 기록으로 이름·메모를 채운다 — 상세에서 AI가 붙인 이름이나 취소한 글이 저장에 섞이지 않는다 (작업 6). 기록과 사진은 한 번에 저장·삭제되어 반쪽(사진만·기록만)이 남지 않고, 저장 공간이 모자라면 한국어로 알린다 (작업 7) |
...
| 백업 내려받기 / 불러오기 (ZIP) | 동작. 비우고 불러와서 기록·사진 3판이 되살아나는 것 확인. 폰에서는 사용자가 '백업했습니다'를 눌러야 "백업됨"으로 적는다 (공유 창·내려받기 결과를 브라우저가 정직하게 알려 주지 않는다). 사진 도중에 끊긴 복원은 같은 파일을 다시 불러오면 이어받는다. 백업에는 긴 변 2048px 축소본이 들어간다 (화면에도 적혀 있다) 파일 안의 기록은 하나씩 검사해 빈 칸은 채우고, id나 시각이 없는 것만 건너뛰고 "읽지 못한 기록 N건은 건너뛰었습니다"로 알린다. 기록은 사진과 함께 한 건씩 한 번에 들어온다 (작업 8) |
