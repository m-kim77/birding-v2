...
/**
 * 카드에 넣을 사진을 불러온다. 주소가 없거나 실패하면 null — 카드는 사진 없이도 그려진다.
 * crossOrigin을 주지 않으면 다른 출처의 사진을 그린 캔버스는 "오염"돼서 내보낼 수 없다.
 */
...
/**
 * 카드를 PNG로 만든다. 모서리 바깥은 투명하다.
 * 사진이 다른 출처에서 와서 캔버스가 오염되면 Error를 던진다 (제품에서는 사진이 기기 안에 있어 생기지 않는다).
 */
export async function cardImage(s: Sighting): Promise<Blob> {
  const [img] = await Promise.all([loadPhoto(s.photo), loadCardFonts(s.speciesKo)])
...
function drawFrame(ctx: CanvasRenderingContext2D, s: Sighting, img: HTMLImageElement | null, t: number): void {
